...
class CaseBase(PrintableModel):
    ...
    persoonlijk_begeleider_telefoonnummer = models.CharField(
        verbose_name=_("Telefoonnummer persoonlijk begeleider"),
        max_length=20,
        blank=True,
        null=True,
    )
    ...
    omklap_client_volgende_stappen_gezet = models.TextField(
        verbose_name=_(
            "Hiervoor worden de volgende stappen gezet (zowel formeel als informeel)"
        ),
        blank=True,
        null=True,
    )
    ...
    omklap_toelichting = models.TextField(
        verbose_name=_(
            "Toelichting (bv. doelen niet behaald maar geen risico voor zelfstandig wonen)"
        ),
        blank=True,
        null=True,
    )
    ...
    match_woning_informatie_delen = models.BooleanField(
        verbose_name=_(
            "Ik deel als zorgaanbieder deze informatie met de woningcorporatie."
        ),
        blank=True,
        null=True,
    )
    ...
    kennismaking_wooncorporatie_goed_huurderschap = models.BooleanField(
        verbose_name=_(
            "Ik verklaar dat ik me als een goed huurder zal gedragen. Dit houdt in dat ik me houd aan de woonregels, geen overlast veroorzaak en mijn huur op tijd betaal."
        ),
        blank=True,
        null=True,
    )
    ...
    kennismaking_wooncorporatie_huisregels = models.TextField(
        verbose_name=_(
            "Zijn er huisregels in het complex waarmee je rekening moet houden?"
        ),
        blank=True,
        null=True,
    )
    kennismaking_wooncorporatie_afval = models.TextField(
        verbose_name=_(
            "Weet je waar je afval kwijt kunt (afvalbak, glasbak, grof vuil voorzieningen)?"
        ),
        blank=True,
        null=True,
    )
    kennismaking_wooncorporatie_technische_klachten = models.TextField(
        verbose_name=_(
            "Weet je hoe je technische klachten kunt melden bij de woningcorporatie?"
        ),
        blank=True,
        null=True,
    )
    ...
    kennismaking_wooncorporatie_overlast_signalen = models.TextField(
        verbose_name=_(
            "Hoe worden signalen van overlast gedeeld? (verwijzing overlastprotocol)"
        ),
        blank=True,
        null=True,
    )
    ...
    kennismaking_wooncorporatie_wijk_voorzieningen_waar = models.TextField(
        verbose_name=_(
            "Weet je waar voorzieningen zijn waar jij gebruik van kunt maken?"
        ),
        blank=True,
        null=True,
    )
    ...
    kennismaking_wooncorporatie_wijk_voorzieningen_anders = models.TextField(
        verbose_name=_(
            "Zijn er nog andere voorzieningen die de woningcorporatiemedewerker of begeleider wil voorstellen?"
        ),
        blank=True,
        null=True,
    )
    kennismaking_wooncorporatie_doelen_wonen_huurderschap = models.TextField(
        verbose_name=_(
            "Waar wil jij aan werken dat met wonen en goed huurderschap te maken heeft?"
        ),
        blank=True,
        null=True,
    )
    kennismaking_wooncorporatie_ondersteuning_medewerker = models.TextField(
        verbose_name=_(
            "Hoe kan de medewerker van de zorgaanbieder en/of de corporatie hierbij ondersteunen?"
        ),
        blank=True,
        null=True,
    )
    kennismaking_wooncorporatie_borg_betalen = models.PositiveSmallIntegerField(
        verbose_name=_(
            "Moet er borg worden betaald als het huurcontract op naam van de bewoner komt te staan?"
        ),
        choices=DEFAULT_YES_OR_NO,
        blank=True,
        null=True,
    ...
    kennismaking_wooncorporatie_akkoord_zorgaanbieder_telefoonnummer = models.CharField(
        verbose_name=_("Telefoonnummer zorgaanbieder"),
        max_length=20,
        blank=True,
        null=True,
    ...
    woonevaluatie_contact_met_buren_gewenst = models.TextField(
        verbose_name=_(
            "Zo nee, wil je wel contact? En heb je daar ondersteuning bij nodig?"
        ),
        blank=True,
        null=True,
    )
    ...
    woonevaluatie_huur_betalen_op_tijd = models.PositiveSmallIntegerField(
        verbose_name=_(
            "Heb je de huur elke maand op tijd betaald aan de zorgaanbieder?"
        ),
        choices=DEFAULT_YES_OR_NO,
        blank=True,
        null=True,
    ...
    woonevaluatie_huur_betalen_regeling = models.TextField(
        verbose_name=_(
            "Is er een regeling getroffen en/of extra ondersteuning geregeld?"
        ),
        blank=True,
        null=True,
    )
    ...
    aanvraag_omklap_steunstructuren = models.TextField(
        verbose_name=_(
            "Op welke steunstructuur / structuren kun je een beroep doen na het moment van omklappen?"
        ),
        blank=True,
        null=True,
    )
    ...
    woonevaluatie_akkoord_zorgaanbieder_telefoonnummer = models.CharField(
        verbose_name=_("Telefoonnummer zorgaanbieder"),
        max_length=20,
        blank=True,
        null=True,
    ...
    def urgentiecriteria_zinvolle_dagbesteding_akkoord_value(self):
        if self.urgentiecriteria_zinvolle_dagbesteding_akkoord:
            return DEFAULT_YES_OR_NO_DICT[
                self.urgentiecriteria_zinvolle_dagbesteding_akkoord
            ]
        return self.EMPTY_VALUE
    ...
    def urgentiecriteria_functioneert_psychisch_akkoord_value(self):
        if self.urgentiecriteria_functioneert_psychisch_akkoord:
            return DEFAULT_YES_OR_NO_DICT[
                self.urgentiecriteria_functioneert_psychisch_akkoord
            ]
        return self.EMPTY_VALUE
    ...
    def urgentiecriteria_is_financieel_stabiel_akkoord_value(self):
        if self.urgentiecriteria_is_financieel_stabiel_akkoord:
            return DEFAULT_YES_OR_NO_DICT[
                self.urgentiecriteria_is_financieel_stabiel_akkoord
            ]
        return self.EMPTY_VALUE
    ...
    def urgentiecriteria_functioneert_sociaal_akkoord_value(self):
        if self.urgentiecriteria_functioneert_sociaal_akkoord:
            return DEFAULT_YES_OR_NO_DICT[
                self.urgentiecriteria_functioneert_sociaal_akkoord
            ]
        return self.EMPTY_VALUE
    ...
    def urgentiecriteria_kinderen_gezonde_akkoord_value(self):
        if self.urgentiecriteria_kinderen_gezonde_akkoord:
            return DEFAULT_YES_OR_NO_DICT[
                self.urgentiecriteria_kinderen_gezonde_akkoord
            ]
        return self.EMPTY_VALUE
    ...
    def status(self, sections):
        ...
        filled_fields = [
            f for f in required_fields if hasattr(self, f) and getattr(self, f)
        ]
        not_filled_fields = [
            FIELDS_DICT.get(f).label
            for f in required_fields
            if hasattr(self, f) and not getattr(self, f)
        ]
        return {
            "percentage": int(
                float(float(len(filled_fields) / len(required_fields))) * 100
            ),
    ...
    def address_complete(self):
        return bool(
            self.adres_straatnaam
            and self.adres_huisnummer
            and self.adres_postcode
            and self.adres_plaatsnaam
        )
...
class Case(CaseBase):
    ...
    def delete_request_seconds_left(self):
        datetime_treshold = timezone.now() - timedelta(
            seconds=config.CASE_DELETE_SECONDS
        )
    ...
    def create_version(self, version):
        case_dict = dict(
            (ff.name, getattr(self, ff.name))
            for ff in Case._meta.get_fields()
            if hasattr(self, ff.name)
            and ff.name in [f.name for f in CaseVersion._meta.get_fields()]
        )
    ...
    def get_linked_federations(self):
        ...
        return Federation.objects.filter(
            federation_id__in=list(
                set([u.federation.federation_id for u in self.get_linked_users()])
            )
        )

    def delete_related(self):
        document_path = os.path.join("uploads", "cases", "%a" % self.id)
    ...
    def address_history(self):
        ...
        return self.case_version_list.filter(id__in=[o.get("id") for o in qs]).order_by(
            "-saved"
        )
    ...
    def get_history(self, form_config):
        ...
        fields = [
            f
            for f in get_sections_fields(form_config.get("sections"))
            if f not in form_config.get("options", {}).get("no_history", [])
        ]
            ...
            k: [
                {
                    ...
                    version_key: CASE_VERSION_BY_SLUG.get(
                        dic[version_key].get("value"), {}
                    ).get("title"),
        ...
        dl = {
            k: [
                ...
                if len(v) - 1 > i
                and v[i].get(value_key) != v[i + 1].get(value_key)
                or len(v) - 1 == i
            ]
            for k, v in dl.items()
        }

        dl = {k: [vv for vv in v if vv.get(value_key) != "—"] for k, v in dl.items()}
        return dl
    ...
    def get_versions(self, form):
        ...
        qs = [
            qs[i + 1].get("case_version")
            for i in range(len(qs))
            if qs[i].get("status") in IN_CONCEPT and len(qs) - 1 > i
        ]
        vl = CaseVersion.objects.filter(id__in=qs).order_by("-created")
        return vl
...
class CaseStatus(models.Model):
    ...
    def html(self):
        ...
        return mark_safe(
            "<div><span>%s</span><small>%s</small><small>%s</small></div>"
            % (
                self.case.id,
                self.form,
